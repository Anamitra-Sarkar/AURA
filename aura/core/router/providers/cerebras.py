...
class CerebrasClient:
    async def complete(self, model_id: str, messages: list[dict[str, object]], max_tokens: int, temperature: float, tools: list[dict[str, object]] | None) -> object:
        if not os.getenv("CEREBRAS_API_KEY"):
            raise ProviderUnavailableError("missing CEREBRAS_API_KEY")
        headers = {
            "Authorization": f"Bearer {os.getenv('CEREBRAS_API_KEY', '')}",
            "Content-Type": "application/json",
        }
        return await post_chat_completion("https://api.cerebras.ai/v1/chat/completions", headers, "cerebras", model_id, messages, max_tokens, temperature, tools)
